"""
MCP Server for Archon (Unified Version with Health Endpoint)

This is the MCP server that uses HTTP calls to other services
instead of importing heavy dependencies directly. Includes built-in
health endpoint using FastMCP's custom_route feature.
...
## Available Functions
- Task Management: create_task, list_tasks, get_task, update_task, delete_task
...
- Research: perform_rag_query, search_code_examples, get_available_sources
"""
...
async def health_endpoint(request: Request) -> JSONResponse:
    ...
    # Add context health status if available
    if _shared_context and hasattr(_shared_context, 'health_status'):
        health_data["health"] = _shared_context.health_status
    else:
        health_data["health"] = {
            "status": "healthy",
            "api_service": True,
...
# Add other custom endpoints for archon-server compatibility
@mcp.custom_route("/clients", methods=["GET"])
async def clients_endpoint(request: Request) -> JSONResponse:
...
def main():
    ...
    try:
        ...
        logger.info("🚀 Starting Archon MCP Server (Unified)")
        logger.info(f"   Mode: {transport_type.upper()}")
        logger.info(f"   Health Endpoint: http://{server_host}:{server_port}/health")
        
        # Different URLs for different transports
        if transport_type == "sse":
            logger.info(f"   MCP URL: http://{server_host}:{server_port}/sse")
        else:
            logger.info(f"   MCP URL: http://{server_host}:{server_port}/mcp")
